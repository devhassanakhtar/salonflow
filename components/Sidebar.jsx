...
const ALL_MENU_ITEMS = [
  ...
  { label: "Staff", href: "/dashboard/staff", icon: UserRoundCog },
];

const generalItems = [{ label: "Settings", href: "/dashboard/settings", icon: Settings }];
...
export default function Sidebar({ isOpen, onClose }) {
  ...
  const menuItems = ALL_MENU_ITEMS.filter((item) => allowedRoutes.includes(item.href));
  const visibleGeneralItems = generalItems.filter((item) => allowedRoutes.includes(item.href));
  ...
  return (
    <>
      {isOpen && (
        <div onClick={onClose} className="fixed inset-0 z-40 bg-slate-900/40 lg:hidden" />
      )}
      ...
      >
        <div className="flex h-[76px] shrink-0 items-center justify-between border-b border-slate-100 px-6">
          <Link href="/dashboard" onClick={handleNavigation} className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-white shadow-sm">
              <Scissors size={18} />
            </div>
            <div>
              <h1 className="text-[17px] font-bold tracking-tight text-slate-900">SalonFlow</h1>
        ...
        <div className="shrink-0 border-t border-slate-100 p-4">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-500 transition hover:bg-red-50 hover:text-red-600 cursor-pointer"
          >
            <LogOut size={18} />
            Logout
          </button>
